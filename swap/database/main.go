...
import (
	...
	"github.com/tomochain/orderbook/common/errors"
)
...
type Database interface {
	// CreateAddressAssociation creates Bitcoin/Ethereum-Tomochain association. `addressIndex`
	// is the chain (Bitcoin/Ethereum) address derivation index (BIP-32).
	CreateAddressAssociation(chain Chain, tomochainAddress, address string, addressIndex uint32) error
	// GetAssociationByChainAddress searches for previously saved Bitcoin/Ethereum-Tomochain association.
	// Should return nil if not found.
	GetAssociationByChainAddress(chain Chain, address string) (*AddressAssociation, error)
	// GetAssociationByTomochainPublicKey searches for previously saved Bitcoin/Ethereum-Tomochain association.
	// Should return nil if not found.
	GetAssociationByTomochainPublicKey(tomochainPublicKey string) (*AddressAssociation, error)
...
type AddressAssociation struct {
	...
	TomochainPublicKey string    `db:"tomochain_public_key"`
