...
func (e *depositEndpoint) handleNewDeposit(ev *types.WebsocketEvent, c *ws.Client) {
	// set default here
	o := &types.AddressAssociationWebsocketPayload{}
	...
	associationAddress, _ := e.depositService.GetAssociationByChainAssociatedAddress(o.Chain, o.AssociatedAddress)
	var addressStr string
	...
	if associationAddress != nil && associationAddress.Status == types.PENDING {
		...
		// now do the left
		address, index, err := e.depositService.GenerateAddress(o.Chain)
		...
		// update association, client should update as well in swarm feed
		// after receiving and displaying the address generated by server
		err = e.depositService.SaveAssociationByChainAddress(o.Chain, address, index, o.AssociatedAddress, o.PairAddresses)
		if err != nil {
			logger.Error(err)
			c.SendMessage(ws.DepositChannel, types.ERROR, err.Error())
			return
	...
	response := &types.GenerateAddressResponse{
		ProtocolVersion: swap.ProtocolVersion,
		Chain:           o.Chain.String(),
		Address:         addressStr,
		Signer:          e.depositService.SignerPublicKey().Hex(),
	}

	// register for this user
	ws.RegisterDepositConnection(o.AssociatedAddress, c)
